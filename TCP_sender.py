...
async def drone_commander(start_time, takeoff_height, traj, duration):
    dummy_drone_id = "06"
    server_port = 6000
    dummy_drone_socket: trio.SocketStream = await establish_connection_with_handler(dummy_drone_id, server_port)
...
async def parent():
    start_time = trio.current_time() + 10
    p0 = [1.4, 0, 0.5]
    p1 = [-0.75, 0, 0.5]
    # p0 = [3, 0, 0.5]
    # p1 = [-3, 0, 0.5]
    ...
    async with trio.open_nursery() as nursery:
        nursery.start_soon(drone_commander, start_time, takeoff_height, traj, duration)
